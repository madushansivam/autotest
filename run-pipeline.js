...
async function generateTestCases(pageData) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (attempt === MAX_RETRIES) throw new Error('Test generation failed after retries: ' + err.message);
    }
  }
}
...
async function generateScript(testCase, pageData) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (code.length >= 10 && !code.includes('<html')) return code;
  }
  return null;
}
...
async function executeTest(page, script, targetUrl) {
  ...
  const result = await testFn(page).catch((err) => ({ passed: false, error: err.message, __harnessCaught: true }));
...
// The full pipeline, callable from the server. Returns the new crawl's DB id.
export async function runFullPipeline(targetUrl) {
  process.on('unhandledRejection', () => {}); // safety net, matches run-tests.js hardening
