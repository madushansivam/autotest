      ...
      <nav className="relative border-b border-white/5 bg-surface-1/50 backdrop-blur-md sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-white tracking-tight">AutoTest</span>
            {activeRuns > 0 && (
              <span className="ml-2 flex items-center gap-1 text-xs text-brand-light bg-brand/10 border border-brand/20 px-2 py-0.5 rounded-full animate-pulse-brand">
                <Spinner size="sm" />
                {activeRuns} running
              </span>
            )}
      ...
      <main className="relative max-w-5xl mx-auto px-4 py-10">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            {runs && runs.length > 0 && (
              <span className="text-sm text-slate-500">{runs.length} run{runs.length !== 1 ? 's' : ''}</span>
            )}
          </div>
